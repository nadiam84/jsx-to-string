...
function serializeItem(item, options) {
  ...
  if (typeof item === 'string') {
    ...
    result = options.detectFunctions ? item.name && item.name !== options.key ? item.name : item.toString() : '...';
  }
  return result;
}
...
function jsxToString(component, options) {
  ...
  if (component.props) {
    ...
    componentData.props = Object.keys(component.props).filter(function (key) {
      ...
      var value = opts.keyValueOverride[key] || serializeItem(component.props[key], _extends({}, opts, { key: key }));
